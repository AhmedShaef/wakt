...
func (s Store) QueryByID(ctx context.Context, workspaceUserID string) (WorkspaceUser, error) {
	...
	if err := database.NamedQueryStruct(ctx, s.log, s.db, q, data, &workspaceUser); err != nil {
		return WorkspaceUser{}, fmt.Errorf("selecting workspaceUserID[%q]: %w", workspaceUserID, err)
	}
...
func (s Store) QueryByuIDwID(ctx context.Context, workspaceID, userID string) (WorkspaceUser, error) {
	...
	var workspaceUser WorkspaceUser
	if err := database.NamedQueryStruct(ctx, s.log, s.db, q, data, &workspaceUser); err != nil {
		return WorkspaceUser{}, fmt.Errorf("selecting workspaceUserID[%q]: %w", workspaceID, err)
	}
